...
void _makise_draw_char(MakiseBuffer *b, uint16_t ind, uint16_t x, uint16_t y, MakiseFont *font, uint32_t c, uint16_t width)
{
    uint32_t bitCounter, rawIndex, colIndex;
    const uint8_t * ptrByte;
    
    ptrByte = &font->table[font->char_index[ind]];
    for(rawIndex = font->height; rawIndex > 0; rawIndex--)
    {
	bitCounter = 0;
	for(colIndex = width; colIndex > 0; colIndex--)
	{
